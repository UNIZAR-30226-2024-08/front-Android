...
export class RegConGoogleComponent implements OnInit{
  ...
  controlarLogin(respuesta: any){
    if(respuesta){
      ...
      this.constantes.usuarioActivo = this.generarNombreUsuario(this.gmailUsuario);

      this.userService.iniciarSesion(this.gmailUsuario, this.generarNombreUsuario(this.gmailUsuario)).subscribe({
        next: (res: any) => {
          console.log(res);
